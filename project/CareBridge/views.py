...
@shared_task
def send_verification_code_task(user_id, purpose="verify"):
    """
    مهمة Celery لإرسال رمز التحقق في الخلفية.
    تستقبل user_id بدلاً من كائن المستخدم.
    """
    try:
        user = User.objects.get(id=user_id)
    except User.DoesNotExist:
        # إذا لم يتم العثور على المستخدم، توقف
        return

    # 1. تحديث الأكواد القديمة
    EmailVerificationCode.objects.filter(user=user, purpose=purpose, is_used=False).update(is_used=True)

    # 2. توليد وحفظ الكود الجديد
    import random
    code = str(random.randint(100000, 999999))
    EmailVerificationCode.objects.create(user=user, code=code, purpose=purpose)
    ...
    # 4. إرسال البريد باستخدام send_mail
    try:
        send_mail(
            subject=subject, # تحتاج إلى تعريف subject ومتغيرات الرسالة
            message=f"{instruction} {code}",
            from_email=settings.DEFAULT_FROM_EMAIL,
            recipient_list=[user.email],
            html_message=html_content, # تحتاج إلى تعريف html_content
            fail_silently=False,
        )
        print(f"Verification code sent successfully to {user.email}")
    except Exception as e:
        # إذا فشل الإرسال، سيسجل الخطأ هنا لكن Gunicorn لن يتأثر
        print(f"Celery Task Failed to send email to {user.email}: {e}")
